...
export default function LeagueDetailPage() {
  ...
  const loadPublicData = useCallback(async () => {
    ...
    try {
      const [leagueData, standingsData, scheduleData] = await Promise.all([
        // Auth token included when signed in so is_registered is populated on first paint
  ...
  const loadRegistrationData = useCallback(async () => {
    ...
    try {
      ...
      // Non-fatal — user might not have a profile yet
    }
    ...
    return (
      <BaseLayout>
        <div className="max-w-3xl mx-auto px-6 py-16 text-center">
          ...
          <Link to="/leagues" className="text-xs text-accent hover:underline">← Back to Leagues</Link>
        </div>
      </BaseLayout>
    );
  }
  ...
  // myRegistration is authoritative once loaded; fall back to league.is_registered
  // while loadRegistrationData is still in-flight to avoid a flash of "not registered"
  ...
  return (
    <BaseLayout>
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        {/* Back link */}
        <Link to="/leagues" className="inline-flex items-center gap-1.5 text-xs text-[#6B6B6B] hover:text-white transition-colors mb-6">
          ← Back to Leagues
        </Link>
