  ...
  test('getInvitation calls correct endpoint', async () => {
    ...
    server.use(
      rest.get('http://localhost:8000/registration/invite/:token', (req, res, ctx) => {
        ...
        return res(ctx.json({ status: 'pending', group_id: 'g1', group_name: 'G', league_id: 'l1', league_name: 'L', inviter_name: 'A', invitee_first_name: 'B', invitee_last_name: 'C', invitee_email: 'b@c.com', expires_at: new Date().toISOString() }));
      })
    );
