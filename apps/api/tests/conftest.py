...
def make_team(db, league_id, name="Test Team", color="#FF0000", **kwargs) -> Team:
...
def make_game(db, league_id, team1_id, team2_id, week=1, **kwargs) -> Game:
    """Create and flush a Game."""
    ...
    defaults = dict(
        ...
        team2_id=team2_id,
        week=week,
        game_date=d(2026, 6, 1),
        game_time="18:00",
        game_datetime=datetime(2026, 6, 1, 18, 0),
        duration_minutes=60,
        status="scheduled",
...
def make_field(db, name="Test Field", **kwargs) -> Field:
    """Create and flush a Field."""
    defaults = dict(
        name=name,
...
def make_field_availability(db, field_id, **kwargs) -> FieldAvailability:
    """Create and flush a FieldAvailability."""
    ...
    defaults.update(kwargs)
    fa = FieldAvailability(**defaults)
    db.add(fa)
    db.flush()
    return fa


def make_league_field(db, league_id, field_id) -> LeagueField:
    """Create and flush a LeagueField association."""
    lf = LeagueField(league_id=league_id, field_id=field_id)
    db.add(lf)
    db.flush()
    return lf
